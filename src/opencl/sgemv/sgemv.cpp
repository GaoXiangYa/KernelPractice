...
void sgemv_v5(const float* A, const float* x, float* y, int M, int N,
  ...
  dm.launch(kernel, global_work_size, local_work_size, "sgemv_v5_kernel");
  dm.read_buffer(by, sizeof(float) * M, y);
}
...
void sgemv_v6(const float* A, const float* x, float* y, int M, int N,
  ...
  auto& dm = DeviceManager::get();
  auto kernel =
      dm.build_kernel("../src/opencl/sgemv/sgemv_v6.cl", "sgemv_v6_kernel");
  ...
  dm.launch(kernel, global_work_size, local_work_size, "sgemv_v6_kernel");
  dm.read_buffer(by, sizeof(float) * M, y);
}

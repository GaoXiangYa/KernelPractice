...
void launch_gemm_q4_1_v0(const unsigned char* A, const float* B, float* C,
                         int M, int N, int K, int block_k, float alpha,
                         float beta) {
...
void launch_gemm_q4_1_v1(const unsigned char* A, const float* B, float* C,
                         ...
                         float beta) {
  auto& dm = DeviceManager::get();
  auto kernel = dm.build_kernel("../src/opencl/gemm_q4_1/gemm_q4_1_v1.cl",
                                "gemm_q4_1_v1_kernel");
  ...
  dm.launch(kernel, global, local, "gemm_q4_1_v1_kernel");
  dm.read_buffer(d_C, sizeof(float) * elems_C, C);
}

void launch_gemm_q4_1_v2(const unsigned char* A, const float* B, float* C,
                         int M, int N, int K, int block_k, float alpha,
                         float beta) {
  auto& dm = DeviceManager::get();
  auto kernel = dm.build_kernel("../src/opencl/gemm_q4_1/gemm_q4_1_v2.cl",
                                "gemm_q4_1_v2_kernel");
  ...
  dm.launch(kernel, global, local, "gemm_q4_1_v2_kernel");
  dm.read_buffer(d_C, sizeof(float) * elems_C, C);
}

void launch_gemm_q4_1_v3(const unsigned char* A, const float* B, float* C,
                         int M, int N, int K, int block_k, float alpha,
                         float beta) {
  auto& dm = DeviceManager::get();
  auto kernel = dm.build_kernel("../src/opencl/gemm_q4_1/gemm_q4_1_v3.cl",
                                "gemm_q4_1_v3_kernel");
  ...
  dm.launch(kernel, global, local, "gemm_q4_1_v3_kernel");
  dm.read_buffer(d_C, sizeof(float) * elems_C, C);
}
...
using RmsnormFunc = void (*)(const std::vector<float>&,
...
static const Variant kVariants[] = {
    ...
    {"v3", launch_rmsnorm_v3, "rmsnorm_v3.cl", "rms_norm_fused_v3", 256, 7},
};

static void bench_one_raw(const Variant& v, const char* label, int N, int D,
                          float epsilon, int warmup, int timed) {
  auto& dm = DeviceManager::get();
  auto kernel =
      dm.build_kernel("../src/opencl/rmsnorm/" + std::string(v.cl_file),
                      v.kernel_name);
  ...
  if (v.args == 4 || v.args == 5) {
    ...
    kernel.setArg(6, cl::Local(8 * sizeof(float)));
  }
  ...
  dm.queue().enqueueNDRangeKernel(kernel, cl::NullRange, global, local, nullptr,
                                  &evt_first);
...
int main() {
  ...
  struct {
    ...
    const char* tag;
      ...
      {256, 256, "small"},  {512, 256, "small"},   {512, 4096, "med "},
      {1024, 4096, "med "}, {2048, 4096, "med "},  {256, 8192, "large"},
      {512, 8192, "large"}, {128, 16384, "xl   "}, {256, 16384, "xl   "},
  };

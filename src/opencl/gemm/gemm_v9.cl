...
__kernel void gemm_v9_kernel(__global const float* A, __global const float* B,
  ...
  __local float shmem_a[2][REG_TILE_M][TILE_M][TILE_K];
  __local float shmem_b[2][REG_TILE_N][TILE_K][TILE_N];
...
#pragma unroll
    for (int m = 0; m < REG_TILE_M; ++m) {
      ...
      if (global_row < M && global_col < K) {
        shmem_a[shmem_cur][m][local_row][local_col] =
            A[global_row * K + global_col];
      } else {
        shmem_a[shmem_cur][m][local_row][local_col] = 0.0f;
      }
    }
...
#pragma unroll
    for (int n = 0; n < REG_TILE_N; ++n) {
      ...
      if (global_row < K && global_col < N) {
        shmem_b[shmem_cur][n][local_row][local_col] =
            B[global_row * N + global_col];
      } else {
        shmem_b[shmem_cur][n][local_row][local_col] = 0.0f;
      }
    }
  }
...
#pragma unroll
      for (int m = 0; m < REG_TILE_M; ++m) {
        ...
        if (global_row < M && global_col < K) {
          shmem_a[shmem_next][m][local_row][local_col] =
              A[global_row * K + global_col];
        } else {
          shmem_a[shmem_next][m][local_row][local_col] = 0.0f;
        }
      }
...
#pragma unroll
      for (int n = 0; n < REG_TILE_N; ++n) {
        ...
        if (global_row < K && global_col < N) {
          shmem_b[shmem_next][n][local_row][local_col] =
              B[global_row * N + global_col];
        } else {
          shmem_b[shmem_next][n][local_row][local_col] = 0.0f;
        }
      }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
...
#pragma unroll
    for (int m = 0; m < REG_TILE_M; ++m) {
      frag_a[reg_cur][m] = shmem_a[shmem_cur][m][local_row][0];
    }

#pragma unroll
    for (int n = 0; n < REG_TILE_N; ++n) {
      frag_b[reg_cur][n] = shmem_b[shmem_cur][n][0][local_col];
    }

    for (int k = 0; k < TILE_K - 1; ++k) {
#pragma unroll
      for (int m = 0; m < REG_TILE_M; ++m) {
        frag_a[reg_next][m] = shmem_a[shmem_cur][m][local_row][k + 1];
      }

#pragma unroll
      for (int n = 0; n < REG_TILE_N; ++n) {
        frag_b[reg_next][n] = shmem_b[shmem_cur][n][k + 1][local_col];
      }

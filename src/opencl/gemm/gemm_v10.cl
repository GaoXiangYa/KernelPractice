...
#define TOTAL_A (REG_TILE_M * TILE_M * TILE_K)
#define TOTAL_B (REG_TILE_N * TILE_K * TILE_N)
...
__kernel void gemm_v10_kernel(__global const float* restrict A,
                              __global const float* restrict B,
                              __global float* restrict C, const int M,
  ...
  // preload tile 0
  {
    int base = 0;
// #pragma unroll
    for (int idx = tid; idx < TOTAL_A; idx += num_threads) {
      ...
      if (global_row < M && global_col < K) {
        shmem_a[shmem_cur][m_block][row * (TILE_K + 1) + col] =
            A[global_row * K + global_col];
      } else {
        shmem_a[shmem_cur][m_block][row * (TILE_K + 1) + col] = 0.0f;
      }
    }

// #pragma unroll
    for (int idx = tid; idx < TOTAL_B; idx += num_threads) {
      ...
      if (global_row < K && global_col < N) {
        shmem_b[shmem_cur][n_block][row * (TILE_N + 1) + col] =
            B[global_row * N + global_col];
      } else {
        shmem_b[shmem_cur][n_block][row * (TILE_N + 1) + col] = 0.0f;
      }
    }
  }
  ...
  for (int ph = 0; ph < num_tiles; ++ph) {
    ...
    if (ph + 1 < num_tiles) {
      ...
      for (int idx = tid; idx < TOTAL_A; idx += num_threads) {
        ...
        if (global_row < M && global_col < K) {
          shmem_a[shmem_next][m_block][row * (TILE_K + 1) + col] =
              A[global_row * K + global_col];
        } else {
          shmem_a[shmem_next][m_block][row * (TILE_K + 1) + col] = 0.0f;
        }
      }
      ...
      for (int idx = tid; idx < TOTAL_B; idx += num_threads) {
        ...
        if (global_row < K && global_col < N) {
          ...
          shmem_b[shmem_next][n_block][row * (TILE_N + 1) + col] = 0.0f;
        }
      }
    }
    // barrier(CLK_LOCAL_MEM_FENCE);
...
#pragma unroll
      for (int m = 0; m < REG_TILE_M; ++m) {
        frag_a[reg_next][m] =
            shmem_a[shmem_cur][m][local_row * (TILE_K + 1) + k + 1];
      }

#pragma unroll
      for (int n = 0; n < REG_TILE_N; ++n) {
        frag_b[reg_next][n] =
            shmem_b[shmem_cur][n][(k + 1) * (TILE_N + 1) + local_col];
      }

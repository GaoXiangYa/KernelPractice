...
__global__ void rmsnorm_v2_kernel(float4 *input, float4 *output, int input_len,
                                  int round, int eps) {
...
void rmsnorm_v2(float *input, float *output, const int input_len,
  ...
  rmsnorm_v2_kernel<<<1, THREAD_COUNT, WARP_NUM * sizeof(float)>>>(input_dev, output_dev, input_len,
                                         round, eps);
...
void rmsnorm_v2_benchmark() {
  std::cout << "rmsborm_v2_benchmark begin: \n";
  ...
  double flops = 3 * input_len + 1024 * 8;
  ...
  for (int i = 0; i < repeat; i++) {
    rmsnorm_v2_kernel<<<1, THREAD_COUNT>>>(input_dev, output_dev, input_len,
                                           round, eps);
  }
  ...
  std::cout << "rmsnorm_v2_benchmark end: \n";
}

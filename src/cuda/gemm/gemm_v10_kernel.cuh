...
template <class Config>
__global__ void gemm_v10_kernel(const float* __restrict__ A,
  ...
  for (int ph = 0; ph < num_tiles; ++ph) {
    ...
    // compute current
    v10::mma<Config>(As_cur, Bs_cur, warp_tile, thread_tile, acc);
  ...
  v10::store_C<Config>(C, warp_tile, thread_tile, acc, M, N, ldc);
}

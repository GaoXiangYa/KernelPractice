...
static __global__ void gemv_kernel_v4(float *mat_a, float *vec_x, float *vec_y,
  ...
  const int row = bx * blockDim.y + ty;
  if (row >= m) {
    return;
  }
  const int y = beta * vec_y[row];
  const int tid = threadIdx.x;
  const int lane_id = tid & (WARP_SIZE - 1);
  const int NUM_WARPS = (n + WARP_SIZE - 1) / WARP_SIZE;
...
void gemv_v4(float *mat_a, float *vec_x, float *vec_y, const int m, const int n,
  ...
  gemv_kernel_v4<<<grid, block, SHARED_MEM_SIZE>>>(
...
void benchmark_gemv_v4(std::ofstream &file, const int m, const int n) {
  ...
  for (int i = 0; i < warmup; ++i) {
    gemv_kernel_v4<<<grid, block, n * sizeof(float)>>>(
        dev_mat_a, dev_vec_x, dev_vec_y, m, n, alpha, beta);
  }
  ...
  file << "GEMV_V4: M = " << m << " N = " << n << " -> " << time_ms << " ms "
       << gflops << " GFlops\n";

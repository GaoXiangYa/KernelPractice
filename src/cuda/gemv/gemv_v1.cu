...
static __global__ void gemv_kernel_v1(float *mat_a, float *vec_x, float *vec_y,
...
void gemv_v1(float *mat_a, float *vec_x, float *vec_y, const int m, const int n,
  ...
  gemv_kernel_v1<BLOCK_SIZE><<<grid, block, SHARED_MEM_SIZE>>>(
...
void benchmark_gemv_v1(std::ofstream &file, const int m, const int n) {
  ...
  for (int i = 0; i < warmup; ++i) {
    gemv_kernel_v1<BLOCK_SIZE><<<grid, block, n * sizeof(float)>>>(
        dev_mat_a, dev_vec_x, dev_vec_y, m, n, alpha, beta);
  }
  ...
  file << "GEMV_V1: M = " << m << " N = " << n << " -> " << time_ms << " ms "
       << gflops << " GFlops\n";

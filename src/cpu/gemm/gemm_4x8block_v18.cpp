...
static void addDot4x8(int k, float *packedA, float *packedB, int ldb, float *C,
  ...
  float *a1_ptr = packedA + k * 8;
  float *a2_ptr = packedA + 2 * k * 8;
  float *a3_ptr = packedA + 3 * k * 8;
...
static void innerKernel(float *packedA, float *B, float *C, int m, int n, int k,
  ...
  for (int j = 0; j < n; j += NR) {
    ...
    for (int i = 0; i < m; i += MR) {
      addDot4x8(k, packedA + i * k * 8, packedB, NR, &C(i, j), ldc);
    }
  }
}
...
void gemm_4x8block_v18(float *A, float *B, float *C, int m, int n, int k) {
  ...
  const int NC = 256;
  const int KC = 128;
  alignas(32) float packedA[MC * KC * 8];
